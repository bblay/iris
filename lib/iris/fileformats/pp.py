...
def _load_cubes_variable_loader(filenames, callback, loading_function):
    ...
    for filename in filenames:
        for field in loading_function(filename):
            # Convert the PPField to a Cube, recording the rules that were used
            rules_result = _load_rules.result(field)
            cube = rules_result.cube
            result_rules_ran = rules_result.matching_rules
            
            # Log the rules used
            iris.fileformats.rules.log('PP_LOAD', filename, result_rules_ran)
            ...
            # Cross referencing
            rules = _cross_reference_rules.matching_rules(field)
            for rule in rules:

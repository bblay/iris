...
class GribWrapper(object):
    ...
    def _compute_extra_keys(self):
        ...
        if gridType=="regular_ll":
            ...
            PSCS = iris.coord_systems.PolarStereographic
            self.extra_keys['_coord_system'] = PSCS(
            ...
            # Note: I think the grib api defaults LaDInDegrees to 60 for grib1.
            LCS = iris.coord_systems.LambertConformal
            self.extra_keys['_coord_system'] = LCS(self.LaDInDegrees,
                                                   self.LoVInDegrees,
                                                   0, 0,
                                                   self.Latin1InDegrees,
                                                   self.Latin2InDegrees,
                                                   ellipsoid=geoid) 
        ...
        if gridType in ["regular_ll", "rotated_ll"]:
            ...
            if not np.all(np.isfinite([x1, y1])):
                raise TranslationError("Could not determine first latlon")
    ...
    def _get_verification_date(self):
        ...
        if self.edition == 1:
            ...
            if time_range_indicator == 0:    time_diff = P1       #Forecast product valid at reference time + P1 P1>0), or Uninitialized analysis product for reference time (P1=0). Or Image product for reference time (P1=0)
                ...
                raise TranslationError("unhandled grib1 timeRangeIndicator = 51 (avg of avgs)")
                ...
                raise TranslationError("unhandled grib1 timeRangeIndicator = %i" %
